...
# 1 "cif.h" 1
  ##
  ## Map from 9 to conditional intensity functions etc
  ##
# 4 "methas.template" 2
# 1 "death.h" 1
# 5 "methas.template" 2

subroutine mh9(par,period,xprop,yprop,mprop,ntypes,
...
while(irep <= nrep) {
 ...
 if(urp>p) {
  ...
  if(urq>q) {
   ...
   call lookup(u,v,m1,x,y,npts,par,period,cifval)
  ...
  ## Propose death:
    ...
    call lookup(x(ix),y(ix),ix,x,y,npts,par,period,cifval)
   ...
   ## Propose shift:
               if(npts != 0) {
                 ...
                 v = yprop(irep)
# 107 "methas.template"
                   call lookup(x(ix),y(ix),ix,x,y,npts,par,period,cvd)

                   call lookup(u,v,ix,x,y,npts,par,period,cvn)
# 119 "methas.template"
