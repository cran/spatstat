...
# 1 "cif.h" 1
  ##
  ## Map from 6 to conditional intensity functions etc
  ##
# 4 "methas.template" 2
# 1 "death.h" 1
# 5 "methas.template" 2

subroutine mh6(par,period,xprop,yprop,mprop,ntypes,
...
while(irep <= nrep) {
 ...
 if(urp>p) {
  ...
  if(urq>q) {
   ...
   call dgs(u,v,m1,x,y,npts,par,period,cifval)
  ...
  ## Propose death:
   ...
   ## call aru(1,zero,one,iseed,xi)
    ...
    call dgs(x(ix),y(ix),ix,x,y,npts,par,period,cifval)
